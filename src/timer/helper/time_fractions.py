...
def calculate_time_fractions(elapsed_time_ns: int) -> ElapsedTimeFractions:
    ...
    microseconds, nanoseconds = divmod(elapsed_time_ns, 1_000)
    # As divmod() can be slow, let's return 0s as a tuple if divmod() isn't needed:
    milliseconds, microseconds = divmod(microseconds, 1_000) if microseconds > 0 else (0, 0)
    seconds, milliseconds = divmod(milliseconds, 1_000) if milliseconds > 0 else (0, 0)
