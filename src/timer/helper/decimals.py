...
def validate_and_normalise(decimals: int | float | None) -> int:
    try:
        if not isinstance(decimals, (int | float)) or decimals is None:
            ...
            print(
                f"{Color.YELLOW}Timer: Decimals set to {MAXIMUM} as the Timer doesn't support more than {MAXIMUM} decimals (i.e. nanoseconds).{Color.OFF}"
            )
            return MAXIMUM
            ...
            print(
                f"{Color.YELLOW}Timer: Decimals {decimals} set to {decimals_rounded} as a floating number is invalid input.{Color.OFF}"
            )
