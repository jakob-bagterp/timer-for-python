...
from mock_data_time_fractions import random_microseconds_as_ns, random_milliseconds_as_ns, random_seconds_as_ns

class TestTimeFractionsCountXToFloat():
    def test_count_microseconds_to_float(self) -> None:
        float_precision = 6
        for _ in range(1000):
            mock_elapsed_time_ns = random_microseconds_as_ns()
            rounded_microseconds_to_float = round(TimeFractions(mock_elapsed_time_ns).count_microseconds_to_float(), float_precision)
    ...
    def test_count_milliseconds_to_float(self) -> None:
        float_precision = 9
        for _ in range(1000):
            mock_elapsed_time_ns = random_milliseconds_as_ns()
            rounded_milliseconds_to_float = round(TimeFractions(mock_elapsed_time_ns).count_milliseconds_to_float(), float_precision)
    ...
    def test_count_seconds_to_float(self) -> None:
        float_precision = 12
        for _ in range(1000):
