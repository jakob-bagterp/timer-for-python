...
def milliseconds_as_ns(milliseconds: int) -> int:
    return milliseconds * 1_000 * microseconds_as_ns(1)


def seconds_as_ns(seconds: int) -> int:
    return seconds * 1_000 * milliseconds_as_ns(1)
...
def random_elapsed_time_ns_and_fractions() -> tuple[int, ElapsedTimeFractions]:
    ...
    milliseconds = randint(0, 1_000 - 1)
    microseconds = randint(0, 1_000 - 1)
    nanoseconds = randint(0, 1_000 - 1)
...
def random_nanoseconds_as_ns(allow_zero: bool = True) -> int:
    return randint(mediate_zero(allow_zero), 1_000 - 1)


def random_microseconds_as_ns(allow_zero: bool = True) -> int:
    random_microseconds = randint(mediate_zero(allow_zero), 1_000 - 1)
...
def random_milliseconds_as_ns(allow_zero: bool = True) -> int:
    random_milliseconds = randint(mediate_zero(allow_zero), 1_000 - 1)
